...
;; "Mak" Tab might complete to "Makefile.am", ignoring the "Makefile.in"
...
;;
;; To enable the mode from .emacs the suggestion is the following setup func
;; which loads and enables the first time you use the minibuffer
;;
...
;; `update-file-autoloads' and friends or M-x package-install.  With them
...
;; -- and it was very inconvenient if loading the code here for other
;; purposes.)
...
(defun completion-ignored-build--completing-file-name-p ()
  "Return true if completing a filename in the minibuffer.
This is `minibuffer-completing-file-name' in Emacs, or a fallback
hack in XEmacs 21."
  (if (eval-when-compile (fboundp 'minibuffer-contents-no-properties))
      minibuffer-completing-file-name   ;; emacs21 up
    ;; xemacs21
    (eq minibuffer-completion-table 'read-directory-name-internal)))

(defun completion-ignored-build--minibuffer-contents ()
  "Return the minibuffer contents, with no text properties.
This is `minibuffer-contents-no-properties' in Emacs, or a
fallback in XEmacs 21."
  (if (eval-when-compile (fboundp 'minibuffer-contents-no-properties))
      (minibuffer-contents-no-properties)   ;; emacs21 up
    ;; xemacs21 -- prompt isn't actual buffer contents
...
* Ignore foo if there's a foo.c, foo.C, foo.cc or foo.cpp (C or
  C++ compiled program).
...
(defun completion-ignored-build-enable-setup ()
  "Enable variable `completion-ignored-build-mode' initially.
This function is designed for use from `minibuffer-setup-hook'.

It enables variable `completion-ignored-build-mode' and then
removes itself from `minibuffer-setup-hook'.  The effect is to
load and enable the mode when you first use the minibuffer.
Waiting for the first minibuffer rather than loading from .emacs
can make Emacs startup a little faster.

`completion-ignored-build-mode' always removes this from
